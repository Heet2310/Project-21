...
function setup() {
  ...
  zombie.addAnimation("zombie_running", zombie_attack);
  zombie.addAnimation("zombie_attack", zombie_running);
  zombie.addImage("zombie_idle", zombie_idle);
  zombie.scale = 0.2;
  zombie.debug = false;
  
  //To create invisible ground on which girl and zombie move
...
function draw() {
  ...
  if (gameState === PLAY) {
    ...
    //Increase speed of ground afer every 100 score
    ...
    //to play the checkpoint sound after every 100 score
    if (score == 0 && score % 100 === 0) {
      checkPointSound.play()
    }

    //to jump the girl when space key is pressed and plays a sound
    ...
    //To stop the moving ground
    ground.velocityX = 0;

    //To stop the moving girl
    girl.velocityY = 0

    //to change the image of girl
    girl.changeImage("girlImage", girlImage);

    //to change animation of zombie
    zombie.changeAnimation("zombie_attack", zombie_attack);

    //to appear that zombie has caught the girl
    zombie.x = girl.x;
    if (zombie.isTouching(girl)) {
      girl.changeImage("girl_collided", girl_collided);
      zombie.changeImage("zombie_idle", zombie_idle);
    }
    //To set lifetime of the game objects so that they are never destroyed
    obstaclesGroup.setLifetimeEach(-1);
    obstaclesGroup.setVelocityXEach(0);

    //To restart when the restart button is pressed
...
function spawnObstacles() {
  if (frameCount % 60 === 0) {
    ...
    obstacle.addImage(obstacle1,obstacle2,obstacle3,obstacle4);
    obstacle.scale = 0.1;
